...
string GetInnerText(Node* pNode)
{
	if (NULL == pNode)
	{
		return NULL;
	...
	while (NULL != pTempNode)
	{
		...
		{
			strContent = strContent + GetInnerText(pTempNode);
		}
...
const char* GetFieldContent(Node* pRootNode, Value& jsonCurrentNode, int iPath, const char** ppPath)
{
	...
	{//
		...
		{// Check the conditions
			// TODO: check all condition, here only check the first one
			Value jsonDefaultValue;
			Value jsonConditionTypeValue = jsonConditionValue.get((UInt)0, jsonDefaultValue)["type"];
			Value jsonConditionCondValue = jsonConditionValue.get((UInt)0, jsonDefaultValue)["cond"];
			Value jsonConditionValueValue = jsonConditionValue.get((UInt)0, jsonDefaultValue)["value"];

			if (!jsonConditionTypeValue.isNull())
			{
				if (strcmp(jsonConditionTypeValue.asCString(), "pre_text") == 0)
				{
					string strResult = GetInnerText(pNode);
					size_t iPos = strResult.find(jsonConditionValueValue.asCString());
					if (string::npos != iPos)
					{
						return strResult.c_str() + iPos + strlen(jsonConditionValueValue.asCString());
					}
				}
			}
		}
		else
		{// Do not have a condition, just return the inner text of the node
			return GetInnerText(pNode).c_str();
		}
...
void test()
{
	...
	bool bRet = RetrieveComment(NULL, pRootNode, jsonRoot);
	if (!bRet)
	{
		puts("Cannot get comment.");
	}
}
